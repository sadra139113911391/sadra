// Copyright (C) (R) 2018 The Nomulus Authors. All Rights Reserved.
//
 ...
 * }</pre>
 */
private final class OteAccountBuilder {
  ...
  private static final ImmutableSortedMap<DateTime, Money> EAP_FEE_SCHEDULE =
      ImmutableSortedMap.of(
          ...
          Money.of(CurrencyUnit.USD, 1000000));
   ...
   * place).
   */
  private static final ImmutableMap<CurrencyUnit, String> DEFAULT_BILLING_ACCOUNT_MAP =
  ...
  private OteAccountBuilder(String baseRegistrarId) {
    ...
    sunriseTld = createTld(baseRegistrarId + "-sunrise", START_DATE_SUNRISE, true, 0);
    gaTld = createTld(baseRegistrarId + "-ga", GENERAL_AVAILABILITY, true, 2);
   ...
   */
  private static OteAccountBuilder forRegistrarId(String baseRegistrarId) {
   ...
   *
   * <p>If true, encountering an existing entity that conflicts with one we want to create will
   * throw an exception during {@link #buildAndPersist}.
   *
   * <p>NOTE that if we fail, no entities are created (the creation is atomic).
   *
   * <p>Default is true (failing if entities exist)
   */
  private OteAccountBuilder setReplaceExisting(boolean replaceExisting) {
   ...
   */
  private OteAccountBuilder addContact(String email) {
   ...
   * . If you want to make this function private, add a check that the value hasn't changed.
   *
  ...
  private OteAccountBuilder setPassword(String password) {
    return transformRegistrars(builder -> builder.setPassword(password));
  }

  /** Sets the client certificate to all the OT&amp;E Registrars. */
  private OteAccountBuilder setCertificate(String asciiCert, DateTime now) {
  ...
  private OteAccountBuilder setIpAllowList(Collection<String> ipAllowList) {
   ...
   */
  private ImmutableMap<String, String> buildAndPersist() {
    saveAllEntities();
   ...
   * to.
   */
  private ImmutableMap<String, String> getRegistrarIdToTldMap() {
    return registrarIdToTld;
  }
  ...
  private static ImmutableMap<String, String> createRegistrarIdToTldMap(String baseRegistrarId) {
    checkArgument(
  ...
  private static String getBaseRegistrarId(String oteRegistrarId) {
